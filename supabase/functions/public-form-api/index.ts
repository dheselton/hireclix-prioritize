...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: proj, error: pe } = await admin.from("pm_projects").insert({
      ...
      visibility: clientRow.is_internal ? "internal_shared" : "client_shared",
      ...
      custom_fields: { request_type: input.requestType ?? "general", ...(input.customFields ?? {}) },
      tags,
    ...
    const { data: task, error: te } = await admin.from("pm_tasks").insert({
      project_id: proj.id,
      title,
      type: input.requestType?.startsWith("dev_") ? "dev" : "design",
      status: "unclaimed",
      priority: "medium",
      duration_days: 1,
      due_date: shipBy,
