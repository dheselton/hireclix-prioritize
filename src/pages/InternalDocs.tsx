...
export default function InternalDocs() {
  ...
  const loadDocs = async () => {
    try {
      ...
      setDocs(data.filter(d => d.audience === "internal"));
    } catch (error) {
      toast({
        title: "Error",
  ...
  const onSubmit = async (data: DocFormValues) => {
    ...
    try {
      ...
      if (editingDoc) {
        ...
        toast({
          title: "Success",
          description: "Internal doc updated successfully",
        });
      } else {
        // Create new doc
        await createDoc({
        ...
        toast({
          title: "Success",
          description: "Internal doc added successfully",
        });
      }
      ...
      toast({
        title: "Error",
        description: editingDoc ? "Failed to update internal doc" : "Failed to add internal doc",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-unbounded font-bold mb-2">Internal Docs</h1>
          <p className="text-muted-foreground">Internal resources, guides, and documentation</p>
        </div>
        <Button 
      ...
      <Dialog open={isDialogOpen} onOpenChange={handleCloseDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingDoc ? "Edit Internal Doc" : "Add Internal Doc"}</DialogTitle>
            <DialogDescription>
              {editingDoc ? "Update the internal documentation item" : "Add a new internal documentation item"}. All fields are required except owner.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., API Integration Guide" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., API, Integration, Technical" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={isSubmitting}>
                  {isSubmitting ? (editingDoc ? "Updating..." : "Adding...") : (editingDoc ? "Update Internal Doc" : "Add Internal Doc")}
                </Button>
              </DialogFooter>
            </form>
          </Form>
